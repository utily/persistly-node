...
describe("Update", () => {
	it("toMongo", () => {
		const argument: persistly.Update<{ name: string, field: number, property: { nested: number }, remove: number }> = { name: "test", field: { $set: 1337 }, property: { nested: 42 }, remove: { $unset: true } }
